...
- **The "click me" hint is a DOM ping, never 3D geometry** (`TelescopePing`). The obvious move — breathing the cyan outline while parked — is a trap: the loop captures the WebGL buffer of the `Telescope` stop, whose Blender reference contains no interface hint, so anything drawn _in the scene_ at rest makes that measurement drift. Cutting it under `prefers-reduced-motion` to save the measurement would have stripped the affordance from motion-sensitive users — letting the test decide the design. In the DOM the question does not arise: the loop does not see the page, and the dot can stay while only the ring goes. Measured after: the `telescope` stop is unchanged at **1.335 %**.
