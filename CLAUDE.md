...
| `design/` | the tests, and a designer in a browser | `screens/*.html` (the oracle of `cv.test.ts` and `bubbleAnchors.test.ts`), `renders/refs/*.png` (the oracle of the comparison loop, and the mockups' backgrounds), `renders/spikes/`, `classic/` (the 2D site's reference prototype — **deliberately NOT in `screens/`**, which `bubbleAnchors.test.ts` globs for `*.html`) |
...
- **The "click me" hint is a DOM marker, never 3D geometry** (`TelescopePing`). It is a **clicking hand** since 2026-08-24, breathing on a steady beat: the dot it replaced said "here", the hand says "here, and click" — the one thing nothing else in the room teaches. Its halo goes through `drop-shadow`, not `box-shadow`, because a box shadow would light the SVG's transparent square instead of the outline. The obvious move — breathing the cyan outline while parked — is a trap: the loop captures the WebGL buffer of the `Telescope` stop, whose Blender reference contains no interface hint, so anything drawn _in the scene_ at rest makes that measurement drift. Cutting it under `prefers-reduced-motion` to save the measurement would have stripped the affordance from motion-sensitive users — letting the test decide the design. In the DOM the question does not arise: the loop does not see the page, so the marker can stay while only its motion goes. Measured after: the `telescope` stop is unchanged at **1.335 %**.
- **Its anchor is measured, not guessed.** The bounding-box centre projects _below_ the frame (y = 898 for a 720-tall viewport) because the telescope reaches down to its foot; at the top of the box the dot floats in the sky and reads as a star. `PING_RISE` puts it on the tube, the only place where it designates the object rather than hovering over it.
- **The two moons swap where nobody can see it — at both ends.** It is the _moment_ that matters, not the condition, and the moment differs each way. Going in, `moonDetailed` turns on when the camera is behind the eyepiece and the screen is **black** (you are looking down the tube), so there is nothing to watch while the texture changes. Coming out, it turns off at the **end** of the return, once the moon is a small disc in the window again. Driven by the phase, the swap was visible on both sides: across the whole window on click, and full-frame on exit.
- **The viewfinder states its own exit.** `Escape` is the only way out — clicking elsewhere does nothing — and nobody guesses a key they are not shown. The hint reuses the project sheet's capped key, lives _inside_ the mask so it fades and scales with it, and sits in a corner: the centre is what you came to look at.
...
- **Green, violet, magenta — a real triad.** The first set paired two blues (cyan and electric blue) with a pink: two neighbours on the colour wheel are not a triad, and the panel read as "blue, varying". `#36ff51` is **eyedropped from the room** (`Mat_CatEyes`, the only true green in the scene); `#e24bc0` is the fan blades' pink pushed to hold its own against it; `#7c46d6` is **new** — no material in the glb carries a deep violet, and without it the wave would cross from green to magenta the short way, i.e. through dirty grey.
- **The blend is by cosine lobes, not by segments.** The first version picked two stops with a `floor()`, so two hard boundaries crossed the cycle and a tile changed regime as it passed one — that is what read as a lack of fluidity. Each stop now carries a lobe centred on it; two always overlap, their sum normalises the result, and the colour moves without ever _arriving_ anywhere.
- **The tint moves the hue, never the luminance.** The first version multiplied the bake by the tint and lifted it: channels clipped, the panel got brighter than Blender authored, and the colours went garish. The tint is now scaled to the bake's luminance before mixing. `LED_TINT` is **measured, not chosen**. With the green/violet/magenta triad the panel is necessarily _more_ saturated than the baked blue — that is the price of the requested chord. What stays bounded is the **luminance**: the more tint, the more the saturated channels clip and the darker the panel gets. Measured at the desk stop: bake 208, tint 0.35 → 194, 0.42 → 190, 0.5 → 185. Past that the panel loses its place in the image.
...
Runtime 2.5D ink, still URL-switchable: `?outline=off|hull|edges|both` (+ `?lw=<px>` live width). `hull` = three OutlineEffect (batched inverted hull, view-dependent silhouettes — takes over rendering via a priority useFrame). `edges` = per-mesh `EdgesGeometry` rendered as screen-space fat lines (`LineSegments2`). Known drei/browser gotchas are commented in the code — read them before refactoring (Html portals, z-index ranges).

**`edges` at 1.4 px is the default since #41** (product arbitration, 2026-08-21, decided on captures) — it is no longer a spike flag, and `?outline=off` is the escape hatch. `hull` and `both` lost, and one of them lost for a reason worth keeping:
...
- Painted correctly, **`hull` covers 0.0–1.0% of the frame** and its Cat panels are indistinguishable from `off`. `HULL_THICKNESS` is in _world_ units, so its apparent width varies across the tour's 84°→7.6° focal span, and unlike `?lw=` it has no live override.
- **`both` adds a defect neither half has** (the Sharmall decal scribbles over). Not isolated.

**Width is the first setting; the exclusion list comes after.** Grease Pencil sits at 2–3 px, but Blender inks a still frame and this is a furnished room: at 2.2 px the stroke was wider than half of what it outlined, and objects were being excluded one by one to compensate for a thickness. Dropping to 1.4 px retired three exclusions on its own, including the keyboard keys — which had never been a design decision, only a casualty.

**The ink is decided PER MATERIAL, not per object** — the same granularity `RoomModel` uses to pick its treatments, and for the same reason: a merged mesh mixes surfaces that have nothing to do with each other. `Poster_Hellfest_Merged` carries the artwork _and_ its frame, so `Poster_: 0` silenced both, and the frame is the thing that should keep its stroke. Verified at runtime across the whole graph: **zero meshes arrive with a material array** (the glTF loader mounts one mesh per primitive), so one mesh is one material and the question can be asked exactly where the answer differs. `Outlines` refuses to ink a mesh that _does_ carry an array — it cannot tell the surfaces apart there, and inking wrongly is worse than not inking.

**Emitters are derived, never listed.** In a baked unlit render an emissive surface _is_ the light; `RoomModel` marks the treatment it chose in `material.userData.runtime` and the ink reads that decision rather than re-deriving it. The rule catches the keyboard backlight, the headset and case LEDs, the NanoLeaf tiles, the cat's eyes, the bulbs — and `Mat_Stars` and `Mat_GuitarString`, which had been listed by hand before anyone looked at what they were made of. It will catch the LED the next export adds.

**The rule describes a RISK, not a prohibition, and `INK_EMITTER_KEEP` is where that shows.** A stroke only puts an emitter out when the emitter is _smaller than the stroke_. Two families are named there with their reasons: the outdoor backdrop (`Mat_Mountains`, `Mat_Treeline`, `Mat_Ground`) uses emissive as flat paint rather than as a lamp, and its silhouette is what makes the landscape — the inked ridge is what gives the window its woodcut; and the fan blades (`Mat_FanBlade`), several centimetres across, where the stroke outlines the petals instead of covering them and gives the case its ten drawn wheels (product decision, 2026-08-21). The blade ink rotates _with_ the blade because `Outlines` parents its lines to the mesh and `DeskAlive` spins that mesh by transform — the difference with the curtain, whose vertices leave in a shader the line geometry never sees.

The three remaining lists are named for their own reason rather than pooled: the stroke is wider than the feature (`Mat_Amp_Grille`, `Mat_Amp_GrilleWire`, the two `Mat_Poster_*` artworks, `Mat_AmpText`), the geometry is dense enough that creases turn to wireframe (`Mat_Moon`, `Mat_MoonDetailed`, `Mat_Sky`), and the geometry moves in a shader so the ink cannot follow (`Mat_Curtain` — measured at the Telescope stop, the stroke detaches and hangs ~15 px out in the night sky). **No capture in `prefers-reduced-motion` can show that last one**, and that is the mode the whole comparison loop runs in — check ink against animated geometry by hand.
...
### Le regard autour du sujet (`src/lib/lookAround.ts` + `src/config/lookAround.ts`)

Issue #133. Parked at a stop, moving the pointer left or right **orbits** the camera a few degrees around the subject, which stays centred.

- **Orbit, not yaw-in-place.** A camera turning on the spot would push the subject out of frame — the opposite of what a look is for. The camera turns around a point _on its own view axis_, at the subject's depth: writing the pivot `P = C + f·d`, a rotation θ about the vertical through `P` gives `C' = P − d·f'`, hence `P = C' + d·f'`. The pivot is still exactly `d` ahead of the new camera, **on any stop, however pitched** — because the rotation is about the WORLD axis and therefore adds no roll. Only what sits _at_ the pivot depth holds still; everything nearer or further slides, and that parallax is the effect.
- **It is composed at write time and NEVER folded into `pose`.** `pose` is what Blender authored and the render's only truth (#115). Adding the orbit to it would let it drift with every mouse move: the next move would start from somewhere nobody composed, the telescope return would aim beside its mark, and nothing would say so. The orbit lives in a scratch pose recomputed each frame.
- **The amplitude is a fraction of the stop's `hfov`, not a fixed degree count**, capped in degrees. What reads as "a little" is not an angle but the share of the frame that moves (≈ `yaw / hfov`), and the tour spans 27° at the CV to 84° at the guitar — one fixed 5° would be three times more visible from one stop to the next. The degree cap exists because a relative amplitude does **not** bound the actual travel: the camera sweeps an arc of `radius × angle`, and on a wide, distant stop 9 % of the field is tens of centimetres. Measured: Desk ±3.9° / 31 cm across, Guitar ±5.7° (capped) / 11.5 cm.
- **The pivot radius is the bubble's anchor depth.** The subject is already declared once, in `src/content/bubbles.ts`; declaring it twice would have been two truths for one thing, and the first to drift would have been the quietest — a wrong pivot breaks nothing, it just slowly slides the subject off-centre while you look at it.
- **`lookAround` is an EXCEPTION list, not a setting to fill in.** The type only admits `false`, so a stop added tomorrow gets the look without anyone thinking about it — the oversight falls the right way. Two stops opt out: **Scoreboard** (author's call — a flat surface you come to _read_, and a few degrees put it in perspective) and **CV** (`CvScreen` is a screen-space DOM panel whose whole job is to read as _displayed by_ the second monitor; the camera orbits, the monitor slides, the CV stays nailed to the viewport). An opted-out stop resolves to radius `0`, so `orbitPose` returns the pose untouched and no caller needs to know about the exception.
- **Cut under `prefers-reduced-motion`, evaluated first** — same rule as the cat's pupils, and for the same reason: something that follows the cursor moves on every mouse motion, for any reason. It is also the second guarantee that the render-comparison loop stays deterministic, the first being that no `pointermove` ever fires in the harness, so the yaw never leaves 0. Measured: 0° and 0 cm at Desk under reduced motion.
- The deadzone is not polish: **without it the framing composed in Blender exists at no moment**, reachable only if the cursor lands on the exact centre column.
- Known, accepted: the Guitar bubble's `--tilted` `-11.15deg` is measured against the amp's edge at the authored framing. Under orbit the amp's projected slope shifts slightly, so that rotation is exact **at cursor-centre only**.

### The narrative bubble (`src/scene/Bubble.tsx`)

The design system's `.bubble` made real (issue #47): a drei `<Html>` re-projects a world-space `anchor` every frame, so the bubble tracks the framed object through camera moves. Two things are load-bearing and commented in the file — the **explicit `portal` prop to App3D's `.bubble-layer`** (without it drei portals into the canvas container and the bubble inherits its stacking context) and the **hardcoded capped `zIndexRange`** (drei's default reaches ~16 million and would paint bubbles above the HUD at 200 and the panel at 300; the layer's z 100 fixes the floor). Prop contract: `visible` drives a 200 ms fade-then-unmount exit, omitting `kicker` switches the markup to the home "inline" variant, and `maxWidth` / `tick` / `tilt` / `className` carry the per-stop design variants. The CSS is split on purpose: anatomy and entry motion live in `src/styles/tokens.css`; `src/styles.css` adds only the projected-layer overrides (`.bubble-layer .bubble` — in-flow, content-box) and the `.bubble--out` exit fade, which beats `bubble-in` at equal specificity **only because `main.tsx` imports tokens.css before styles.css** — never reorder those imports. `BUBBLE_OUT_MS` must equal `--t-bubble-out`, and `tests/bubble.test.ts` is the only thing linking the two (it does not guard the import order). **The bubble is a dialogue box since #122** (the decision is recorded, dated and reversed, in `docs/DESIGN.md`): it says several sentences and the visitor advances them, by click or by wheel. It still opens nothing, and it still carries `pointer-events: none` — **the stage listens, not the bubble**. The rule is _the scene first, the dialogue otherwise_: a click aimed at an interactive object belongs to it (the telescope takes off, a folder flies), anywhere else it makes the stop speak. A bubble that captured its own clicks would swallow the ones meant for the object it points at, since it is anchored right beside it.

**N sentences = N gestures.** A gesture closes the current page; on the LAST one, the same gesture closes it _and_ steps to the next stop. That is what keeps Home — a single sentence — at one scroll, which is the site's first impression. A gesture that lands WHILE the page is typing finishes the typing and does only that; it takes one more to advance (arbitration A, 2026-08-24). The timer never advances a page: the reader owns the pace.

Only forward gestures feed the dialogue — going back is an intent to return, not to read — and only while PARKED: during a move you are already rushing, and arbitrating the dialogue of a stop you have not reached yet means nothing. Arrow keys always traverse the tour. Accessibility is issue #49.
...
**The language is chosen here, and both portfolios inherit it** (2026-08-25). This screen was the only one with no toggle: you arrived in whatever the browser guessed, read the question and both promises in that language, and could only change it _after_ committing to an experience. `LangToggle` is therefore shared — one component, `src/ui/LangToggle.tsx`, on the choice screen and in the classic top bar, at the same corner of both. **Nothing transports the decision**: `useLocale` is a global, persisted store, and `resolveLocale` puts a stored choice ahead of any detection, so the choice holds for the 3D scene, for the classic site, and for the next visit. A language prop coming down from anywhere would be a second truth; `tests/classic.test.ts` fails if one appears. The 3D bar keeps its own vertical `.menu__lang` — same decision, different layout.
...
Issue #29, and the branch of the preselection gate that is **not** the room. It recreates `design/classic/portfolio-2d.dc.html`; the spec that governs it — tokens, timings, easings, the reveal-delay table — is **`docs/PORTFOLIO_2D.md`**, and it is high-fidelity on purpose.

- **No WebGL, ever, and the wireframe cube is where that bites.** The spec records that the author wanted the cubes rebuilt in react-three-fiber. This route cannot: `App.tsx` lazy-imports `App3D` precisely so three/r3f never ship here, and this is the route `resolveExperience` sends the visitor **whose browser has no WebGL context**. Mounting a canvas would hand a WebGL context to the one person who proved they cannot get one — and bill 500 KB of engine to everyone who chose not to load it. So the cubes are CSS 3D (`WireCube.tsx`), six divs with a cyan border, which cannot fail.
- **`styles.css` locks `html, body, #root` to `height: 100%; overflow: hidden`** — right for a full-frame 3D scene, and exactly what makes a one-page site impossible. The classic route unlocks it with `:root[data-experience='classic']`, written by `App.tsx` **beside `lang`** (one writer for root attributes) and removed on leave. Without it the page does not scroll **at all**: the IntersectionObserver never fires, the halo never drifts, the mini-name never appears — and nothing says so; the page merely looks truncated. `tests/classic.test.ts` guards both halves together, because each is inert without the other.
- **The content is `cv.ts` + `projects.ts`, shared with the 3D** (product decision, 2026-08-24). Nothing this page displays is written in it. A second CV copied here would have diverged from the first at the first corrected date, silently. What _is_ local — kicker, tagline, section labels, footer — lives in `UI.classic`, and `tests/classic.test.ts` fails if a company or school name appears there.
  - Two shared-data consequences, both deliberate: `CvGlyph` gained an optional **`initial`** (the 2D tiles need `Pg`, `Ts`, `Tw` — `TypeScript` and `Tailwind CSS` start with the same letter), read through `glyphMark()` so **both** experiences show the same mark; the 3D CV's technical tiles therefore now read `Pg` rather than `P`. **That screen's mockup, `design/screens/02-cv.html`, still renders single letters** — and already carries the collision this solves (TypeScript and Three.js both show `T`). No test compares the two, so nothing breaks; but the app and its design oracle now differ on that strip, and reverting is one line (drop `initial` from `CV.skills`, keep it on the type — the 2D falls back to single letters and loses `Js`/`Tw`/`Pg`). And `CV.identity.age` exists for the classic hero's meta line, which the 3D CV does not display.
- **The decrypt is one implementation, in `src/ui/Scrambled.tsx`** — extracted from `CvScreen` rather than written twice. The rule (settle left to right, never scramble a space, deterministic glyph draw) is there; what varies is passed in. The classic hero uses `DECRYPT_CHARSET_CODE`, which has **no letters**: the CV scrambles job titles, the hero scrambles a proper name at 92 px, and with letters in the set you read plausible near-names for 1.7 s instead of watching a name resolve. The clock lives in `src/lib/decrypt.ts` and not beside the component — a file exporting both a component and a hook loses Fast Refresh, and `--max-warnings 0` makes that an error.
- **Reduced motion cuts the two rAF loops in JS, not just the CSS.** The prototype used `*{animation-duration:.01s}`, which reaches only CSS: its halo and its cube kept turning — a page advertised as still, whose background drifted. House rule applies (cut what starts by itself, keep what a gesture triggers), so the hover accordion, the tile halos and the card lift all stay. The halo stays **painted** at its start framing, and `CUBE_REST_DEG` exists for this path alone: frozen at 0° the cube reads as a truncated pyramid, at 28° as a volume. It is also the loop's starting angle, or the cube would jump on the first frame.
- **The reveal observer's 9999 px top margin is not a trigger tweak, it is a CATCH-UP.** Without it a section skipped in one gesture — fast scroll, the End key, an anchor — never enters the observer's field and stays at `opacity: 0`, permanently, saying nothing.
- **`--classic-width` is 1088 px, not the 1040 the spec writes.** The prototype has no `box-sizing` reset and gets 1040 px of _content_; `styles.css` sets a global `border-box`, where the same declaration measures 992 — 48 px narrower than what the design session framed. The column is what is authored, not the declaration.
- **A reveal group must never contain another one.** The CSS reveals through `[data-revealed='true'] .classic-reveal`, a _descendant_ selector, so a nested group lights with its parent whatever its own observer says. The CV shipped that way for a day: its three blocks fired ~1500 px early, and the symptom was not "it lights too soon" but **"there is no animation at all"**, because you arrive after it is over. Nothing in the DOM shows it — only scrolling does — so `useReveal` `console.warn`s in dev, and `tests/classic.test.ts` guards the specific regression.
- **The language toggle shows both languages, both clickable.** A single button reading `EN` while the page is French says neither which state you are in nor which languages exist — `EN` reads equally as "you are in English" and "switch to English", two exactly opposite readings. The pair _is_ the list; the active side stays a live button because it is also the indicator. Same anatomy as `.menu__lang` in the 3D bar.
- **The hero carries the bubble's cue laser** (#131): the tagline gets **one slow pass** (`classic-cue-pass`, the crossing occupies the whole duration — the bubble's keyframes cross in 22 % then wait, which only makes sense in a loop), and « défiler » — the one word on the page that _asks_ for something — keeps the accent and loops the bubble's own `cue-laser` on a shorter cycle. The gradient recipe is duplicated in `classic.css`; the **motion** is not, and a test asserts the two gradients have not drifted. Reduced motion drops the sweep and keeps the colour, as in the bubble.
- **Section numbers (`01 —`, `02 —`…) are written, unlike the tour's bubbles**, which `bubbleKicker()` computes. The tour reorders — `cameraStops.ts` invites it — while a one-page site has the order of its own scroll, and only a rewrite of the page can change it.

The mobile breakpoint (720 px) is implemented as specced even though the mobile batch is deferred: it is layout, and retrofitting it later would mean rewriting whatever was validated on desktop meanwhile.
