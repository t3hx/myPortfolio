...
- **Speeds are desynchronised and directions alternate.** Ten identical discs at one speed read as a single mechanical part, which a PC case is not. The draw is deterministic (indexed on rank) so the scene stays reproducible.
...
- **What makes smoke is the overlap, not the puff.** Calibrated by measurement: at 22 puffs they did not touch and read as isolated dots floating above the cup. Sixty-four smaller, fainter puffs overlap into a plume. Raise the count before raising the opacity.
