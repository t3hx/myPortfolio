...
- **The telescope outlines on hover** (`TelescopeHover`), reusing the folders' ink from #81 — it was the only interactive object of its stop and said nothing. The hover flag travels through the store rather than a second `<primitive>`: rendering the same `Object3D` twice would **reparent** it out of the scene `RoomModel` owns.
