...
- **Le halo de survol est spécifié mais non maquetté** : aucun écran de `screens/` ne porte `.bubble--interactive`, faute d'objet cliquable arrêté au moment de la session. Le CSS existe et vaut spécification — ce n'est pas une maquette qu'on aurait oubliée. **Tranché avec #47 : aucune bulle n'est interactive** — la bulle est purement narrative, n'ouvre rien, et laisse passer la molette. `.bubble--interactive` reste une spécification dormante si un futur arrêt en a besoin.
- **Précision apportée par #49 : la phrase reprend les événements pointeur, elle seule.** L'accessibilité demande un texte sélectionnable, ce qu'un `pointer-events: none` interdit par construction. La levée est donc portée par `.bubble__text` dans l'app (`src/styles.css`), pas par tokens.css : le fond, le point et le trait de rappel restent transparents aux clics. La molette continue de commander le tour — son écouteur est sur `.stage` et l'événement y remonte depuis la bulle (vérifié au navigateur : roulette au-dessus de la phrase, l'arrêt suivant part). Ce que ça coûte : un clic sur la phrase ne redescend plus au raycaster de la scène. La bulle n'en devient pas cliquable pour autant — ni curseur « main », ni état de survol.
- **Le texte peint est `aria-hidden`** (#49) : il est dit par une région live dédiée (`<StopAnnouncer>`) à l'arrivée à l'arrêt. Une seule copie de la phrase dans l'arbre d'accessibilité, sinon elle serait annoncée puis relue.
